...
import algorithm.assignment.AssignmentSolver.Hungarian;
import algorithm.assignment.AssignmentSolver.VolgenantJonker;

class VolgenantJonkerAlgorithm {
	/************************************************************************
	 *
    ...
    public VolgenantJonkerAlgorithm(double[][] costMatrix){
	...
	private void makeSquare() {
		// matrix already is square
		if (this.costMatrix.length != this.costMatrix[0].length) {
			// fill with zeroes
			int dim = costMatrix.length;
			if (costMatrix[0].length > dim) {
				dim = costMatrix[0].length;
			}
	...
	public int[] execute() {
		...
		//delete rows that were not there?
		int[] sol = new int[originalMatrix.length];
		if(originalMatrix.length > originalMatrix[0].length)
		{
			//-1 to unmatched elements
			for(int i = 0; i< sol.length; i++)
			{
				if(this.rowsol[i]<originalMatrix[0].length)
				{
					sol[i] = this.rowsol[i];
				}
				else
		...
		else // originalMatrix.length < originalMatrix[0].length
		{
			for(int i = 0; i< sol.length; i++)
			{
				sol[i] = this.rowsol[i];
	...
	{
		runningTimeTest(1000,100,100);
		//test(10);
	}

	private static void runningTimeTest(int n, int rows, int cols) {
		Random rand = new Random();
		double[][] costMatrixTest = new double[rows][cols];
		for (int i = 0; i < costMatrixTest.length; i++) {
			for (int j = 0; j < costMatrixTest[i].length; j++) {
				costMatrixTest[i][j] = ((double) rand.nextInt(1000)) / 100.0;
			}
		}
		long start = System.nanoTime();
		for (int c = 0; c < n; c++) {
			Hungarian h = new Hungarian();
			h.minimumCost(costMatrixTest);
		}
		long end = System.nanoTime();
		long hungTime = end -start;
		
		long startv = System.nanoTime();
		for (int c = 0; c < n; c++) {
			VolgenantJonker vj = new VolgenantJonker();
			vj.minimumCost(costMatrixTest);
		}
		long endv = System.nanoTime();
		long vTime = endv -startv;
		
		System.out.print("Hungarian: "+ hungTime/1000000 +  " ms");
		System.out.println("; Vol: "+ vTime/1000000 +  " ms");
	}
	...
	public static void test(int n) {
		for (int c = 0; c < n; c++) {
			...
			double[][] costMatrixTest = new double[r][col];
			for (int i = 0; i < costMatrixTest.length; i++) {
				for (int j = 0; j < costMatrixTest[i].length; j++) {
					costMatrixTest[i][j] = ((double) rand.nextInt(1000)) / 100.0;
				}
			...
			System.out.println("Dimenisions: " + costMatrixTest.length + " x " + costMatrixTest[0].length);

			hSol = h.minimumCost(costMatrixTest);
			vjSol = vj.minimumCost(costMatrixTest);
			System.out.println("Vol " + vjSol + "; Hun " + hSol);
